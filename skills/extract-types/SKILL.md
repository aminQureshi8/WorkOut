...
2. **Directory Placement:** All types must be written into a dedicated file or feature folder inside the `src/types/` directory.
   ...
   - For a specific feature or a complex component, create a matching types file (e.g., `src/types/components.ts` or `src/types/index.ts`).
...
Instead of generating this inline:
```tsx
// ❌ BAD
