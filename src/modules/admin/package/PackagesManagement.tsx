...
// Import the IPackage interface that matches the server‑side model
import { IPackage } from "../../../../model/Package";

/**
 * Local UI representation of a package. In a real app this would be fetched
 * from the API and would directly use {@link IPackage}. For the purpose of the
 * demo we keep a hard‑coded list that mirrors the shape of the Mongoose model.
 */
type PackageItem = IPackage;

// Mock data that follows the shape of the model. In production replace this
// with a fetch call to `GET /api/admin/packages`.
const mockPackages: PackageItem[] = [
  {
    // Mongoose Document fields (only _id is needed for React keys)
    _id: "1",
    name: "بسته پایه",
    slug: "basic-package",
    tagline: "پایه",
...
export default function PackagesManagement() {
  ...
  const handleSelectAll = () => {
    if (selectedPackages.length === mockPackages.length) {
      setSelectedPackages([]);
    } else {
      setSelectedPackages(mockPackages.map((p) => p._id));
    }
  };
  ...
  const getPackageBadge = (tier: string) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full border text-xs ${styles[tier as keyof typeof styles]}`}
      >
  ...
  const totalUsers = mockPackages.reduce((sum, pkg) => sum + pkg.studentCount, 0);
  ...
  // Simple client‑side filter – in a real app this would be done server‑side
    ...
    <div className="min-h-screen bg-gradient-to-br bg-black/30 p-4 md:p-8" dir="rtl">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl mb-2 text-white" style={{ fontFamily: "Marbeh, sans-serif" }}>
              مدیریت پکیج‌ها
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
            ...
            <div className="text-3xl text-white mb-1" style={{ fontFamily: "Marbeh, sans-serif" }}>
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
            ...
            <div className="text-2xl text-white mb-1" style={{ fontFamily: "Marbeh, sans-serif" }}>
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6">
            ...
            <div className="text-xl text-white mb-1" style={{ fontFamily: "Marbeh, sans-serif" }}>
              بسته حرفه‌ای
            </div>
            <div className="text-green-400 text-sm">
          ...
          {filteredPackages.map((pkg) => (
            ...
            >
              ...
              <div className="p-6 border-b border-white/10">
                ...
                <h3 className="text-2xl text-white mb-2" style={{ fontFamily: "Marbeh, sans-serif" }}>
                  {pkg.name}
                </h3>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-white/10 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b border-white/10 flex items-center justify-between sticky top-0 bg-gray-900/80 backdrop-blur-lg">
                <h2 className="text-2xl text-white" style={{ fontFamily: "Marbeh, sans-serif" }}>
                  ایجاد پکیج جدید
                </h2>
              ...
              <div className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="مثال: بسته طلایی" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                  </div>
                  {/* Slug */}
                  <div>
                    <label className="block text-white mb-2">اسلاگ (URL)</label>
                    <input type="text" placeholder="مثال: gold-package" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                  </div>
                  {/* Tagline */}
                  <div>
                    <label className="block text-white mb-2">تاگلاین</label>
                    <input type="text" placeholder="توضیح کوتاه" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                  </div>
                  {/* Description */}
                  <div>
                    <label className="block text-white mb-2">توضیحات</label>
                    <textarea rows={3} placeholder="توضیحات کوتاه درباره پکیج..." className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50 resize-none" />
                  </div>
                  {/* Prices */}
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت یک ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت سه ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت شش ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                  </div>
                  {/* Original Prices */}
                  <div className="grid grid-cols-3 gap-4 mt-2">
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت اصلی یک ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت اصلی سه ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2 text-sm">قیمت اصلی شش ماهه</label>
                      <input type="number" placeholder="۰" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                  </div>
                  {/* Icon & Colour */}
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    <div>
                      <label className="block text-white mb-2">آیکون (URL یا کلاس)</label>
                      <input type="text" placeholder="آیکن یا کلاس" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2">کلاس رنگ</label>
                      <input type="text" placeholder="مثال: bg-blue-500" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                  </div>
                  {/* Rating & Review Count */}
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    <div>
                      <label className="block text-white mb-2">امتیاز (0‑5)</label>
                      <input type="number" min={0} max={5} placeholder="0" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                    <div>
                      <label className="block text-white mb-2">تعداد نظرات</label>
                      <input type="number" placeholder="0" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:textwhite/40 focus:outline-none focus:border-orange-500/50" />
                    </div>
                  </div>
                  {/* Student Count */}
                  <div>
                    <label className="block text-white mb-2">تعداد دانش‌آموزان فعال</label>
                    <input type="number" placeholder="0" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:text-white/40 focus:outline-none focus:border-orange-500/50" />
                  </div>
                  ...
                  {/* Features */}
                  <div>
                    <label className="block text-white mb-2">امکانات (هر خط یک مورد)</label>
                    <textarea rows={6} placeholder="دسترسی به برنامه تمرینی\nرژیم غذایی شخصی‌سازی شده\nپشتیبانی ۲۴/۷" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder:textwhite/40 focus:outline-none focus:border-orange-500/50 resize-none" />
                  </div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button onClick={() => setShowCreateModal(false)} className="px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-lg transition-colors">
                    انصراف
                  </button>
                </div>
              </div>
